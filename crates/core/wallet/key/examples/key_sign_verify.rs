...
use key::key_old;
use key_old::{
    CurveType, generate_karix_address, import_from_private_key, sign_message_ed25519,
    sign_message_k256, sign_message_p256, verify_signature, verify_signature_ed25519,
...
fn main() {
    ...
    let (private_key_p256, public_address_p256, seed_phrase_p256) =
        generate_karix_address(12, CurveType::P256);
    ...
    if !p256_signature.is_empty() {
        ...
        match verify_signature_p256(address_hex, message_p256, &p256_signature) {
            Ok(true) => {
                println!("✅ Signature verification successful with P256-specific function!")
            }
    ...
    match import_from_private_key(&private_key_p256, CurveType::P256) {
        Ok((imported_private_key, _, imported_public_address)) => {
            ...
            if public_address_p256 == imported_public_address {
                ...
                println!(
                    "❌ Address mismatch: {} vs {}",
                    public_address_p256, imported_public_address
                );
            }
            ...
            match sign_message_p256(&imported_private_key, message_p256) {
                Ok(sig) => {
                    ...
                    match verify_signature(&imported_public_address, message_p256, &sig) {
                        Ok(true) => {
                            println!("✅ Signature verification successful with generic function!")
                        }
                        Ok(false) => {
                            println!("❌ Signature verification failed with generic function!")
                        }
                        Err(e) => {
                            println!("Error verifying signature with generic function: {}", e)
                        }
                    }
                    ...
                    match verify_signature_p256(address_hex, message_p256, &sig) {
                        Ok(true) => println!(
                            "✅ Signature verification successful with P256-specific function!"
                        ),
                        Ok(false) => println!(
                            "❌ Signature verification failed with P256-specific function!"
                        ),
                        Err(e) => println!("Error verifying with P256-specific function: {}", e),
                    }
                }
    ...
    if !ed25519_signature.is_empty() {
        ...
        match verify_signature_ed25519(address_hex, message_ed25519, &ed25519_signature) {
            Ok(true) => {
                println!("✅ Signature verification successful with Ed25519-specific function!")
            }
            Ok(false) => {
                println!("❌ Signature verification failed with Ed25519-specific function!")
            }
    ...
    match import_from_private_key(&private_key_ed25519, CurveType::Ed25519) {
        Ok((imported_private_key, _, imported_public_address)) => {
            ...
            if public_address_ed25519 == imported_public_address {
                ...
                println!(
                    "❌ Address mismatch: {} vs {}",
                    public_address_ed25519, imported_public_address
                );
            }
            ...
            match sign_message_ed25519(&imported_private_key, message_ed25519) {
                Ok(sig) => {
                    ...
                    match verify_signature_ed25519(address_hex, message_ed25519, &sig) {
                        Ok(true) => println!(
                            "✅ Signature verification successful with Ed25519-specific function!"
                        ),
                        Ok(false) => println!(
                            "❌ Signature verification failed with Ed25519-specific function!"
                        ),
                        Err(e) => println!("Error verifying with Ed25519-specific function: {}", e),
                    }
                }
