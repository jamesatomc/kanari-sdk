...
module kanari_system::object {
    ...
    struct UID has store, drop { addr: address }
    ...
    public fun uid_to_inner(uid: &UID): ID { ID { bytes: uid.addr } }
    public fun id_from_address(bytes: address): ID { ID { bytes } }
    public fun id_to_address(id: &ID): address { id.bytes }
    public fun id_to_bytes(id: &ID): vector<u8> { signer::address_to_bytes(id.bytes) }
    public fun uid_address(u: &UID): address { u.addr }
    public fun uid_to_u64(u: &UID): u64 { signer::address_to_u64(u.addr) }
    public fun uid_to_bytes(u: &UID): vector<u8> { signer::address_to_bytes(u.addr) }
    public fun id_bytes(u: &UID): vector<u8> { signer::address_to_bytes(u.addr) }
    ...
    public fun delete(id: UID) { delete_impl(id); }
