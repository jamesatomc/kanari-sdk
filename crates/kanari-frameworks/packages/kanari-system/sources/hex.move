...
module kanari_system::hex {
    use std::vector;

    const EInvalidHexLength: u64 = 0;
    const ENotValidHexCharacter: u64 = 1;
    /// Input exceeds `MAX_HEX_LENGTH`.
    const EHexTooLong: u64 = 2;
    /// Cap on hex encode/decode inputs to bound O(n) alloc / gas grief (64KB).
    const MAX_HEX_LENGTH: u64 = 65536;
    ...
    /// Aborts with `EHexTooLong` when input exceeds 64KB.
    public fun encode(bytes: vector<u8>): vector<u8> {
        let (i, r, l) = (0, vector[], vector::length(&bytes));
        assert!(l <= MAX_HEX_LENGTH, EHexTooLong);
        let hex_vector = HEX;
        while (i < l) {
            vector::append(
                &mut r,
