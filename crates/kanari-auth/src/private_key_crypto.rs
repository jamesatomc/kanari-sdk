...
fn derive_key(password: &[u8], salt: &[u8], iterations: u32) -> AuthResult<Zeroizing<[u8; 32]>> {
    ...
    hasher.update(salt);
    let mut block = Zeroizing::new(hasher.finalize().to_vec());

    for _ in 1..iterations {
        let mut round = Sha256::new();
        round.update(block.as_slice());
        round.update(password);
        round.update(salt);
        block = Zeroizing::new(round.finalize().to_vec());
    }
...
mod tests {
    ...
    fn encrypted_key_round_trip_still_works() {
        ...
        assert_eq!(decrypt_private_key(&encrypted, "password").unwrap(), "secret");
    }
}
