...
fn pending_key(digest: &[u8]) -> Vec<u8> {
    [b"mempool:object:tx:".as_slice(), hex::encode(digest).as_bytes()].concat()
}

fn executed_key(digest: &[u8]) -> Vec<u8> {
    [b"object_tx:executed:".as_slice(), hex::encode(digest).as_bytes()].concat()
}
...
impl BlockchainEngine {
    ...
    pub fn pending_object_transactions(&self) -> Result<Vec<SignedObjectTransaction>> {
        ...
        for digest in index {
            if let Some(tx) = state.store.load(&pending_key(&digest))? {
                out.push(tx);
            }
        }
        Ok(out)
    ...
    pub fn finalize_object_transaction(&self, digest: &[u8]) -> Result<Option<SignedObjectTransaction>> {
    ...
    fn finish_object_transaction(
        ...
        let mut locks: LockMap = store.load(LOCKS_KEY)?.unwrap_or_default();
        release_locks(&mut locks, &tx, digest);
        let mut updates = vec![
            (INDEX_KEY.to_vec(), bcs::to_bytes(&index)?),
            (LOCKS_KEY.to_vec(), bcs::to_bytes(&locks)?),
        ];
