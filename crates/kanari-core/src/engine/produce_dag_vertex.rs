...
impl DagEngine {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            authority_id,
            state_cache: Arc::new(RwLock::new(LruCache::new(NonZeroUsize::new(10).expect("Cache size must be > 0")))),
        })
    }
    ...
    pub fn produce_vertex(&self) -> Result<DagBlockInfo> {
        ...
        let (transactions, tx_to_remove_from_pending) = {
            ...
            let chain = self.engine.blockchain.read().unwrap_or_else(|e| e.into_inner());
            let pending = self.engine.pending_txs.read().unwrap_or_else(|e| e.into_inner());
        ...
        if !tx_to_remove_from_pending.is_empty() {
            let mut pending = self.engine.pending_txs.write().unwrap_or_else(|e| e.into_inner());
        ...
        let (executed_state, state_root, executed, failed) = {
            ...
            let root = state_arc.write().unwrap_or_else(|e| e.into_inner()).compute_state_root();
