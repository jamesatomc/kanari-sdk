...
use mona_crypto::{HashAlgorithm, hash_data_blake3, hash_data_with_algorithm, secure_clear};
...
pub fn process_blockchain_transfer(
    ...
    let result = match crate::transfer_tokens::transfer_tokens(
        from_address,
        to_address,
        amount,
        &password_copy,
        ...
        Ok(transaction) => {
            ...
            if add_pending_transaction(transaction.clone()) {
                info!(
                    "Transaction added to pending queue: {} -> {}, amount: {}, gas fee: {}",
