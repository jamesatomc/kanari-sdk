...
impl VMTransaction {
    pub fn new(
        ...
        hasher.update(
            format!(
                "{}{}{}{}{}",
                sender, module_id, function, timestamp, gas_budget
            )
            .as_bytes(),
        );
...
pub fn execute_vm_transaction(tx: &VMTransaction) -> Result<JsonValue, String> {
    ...
    let module = {
        let state = VM_STATE
            .read()
            .map_err(|e| format!("VM state access failed: {}", e))?;
    ...
    if !module
        .public_functions
        .iter()
        .any(|f| f.to_lowercase() == function_lower)
    {
        return Err(format!(
            "Function '{}' not found in module {}",
            tx.function, module.module_id
        ));
    }
