...
def apply() -> None:
    crypto = "crates/kanari-auth/src/private_key_crypto.rs"
    write(
        crypto,
        r'''use aes_gcm::{
    Aes256Gcm, KeyInit,
...
''',
    ...
    auth = "crates/kanari-auth/src/auth_manager.rs"
    text = read(auth)
    text = text.replace(
    ...
    if old not in text:
        raise RuntimeError("auth login decrypt block not found")
    write(auth, text.replace(old, new, 1))

    session = "crates/kanari-auth/src/session.rs"
    text = read(session)
    text = text.replace(
    ...
    old = '''    pub fn validate_session(&mut self, session_id: &str) -> Result<&Session, AuthError> {
        let session = self
            .sessions
            .get(session_id)
    ...
    new = '''    pub fn validate_session(&mut self, session_id: &str) -> Result<&Session, AuthError> {
        let expired = self
            .sessions
            .get(session_id)
            .ok_or(AuthError::InvalidSession)?
    ...
    if old not in text:
        raise RuntimeError("session validation block not found")
    write(session, text.replace(old, new, 1))
